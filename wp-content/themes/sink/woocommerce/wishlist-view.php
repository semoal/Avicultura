                ...
                <div class="wishlist-title <?php echo ( $is_custom_list ) ? 'wishlist-title-with-form' : '' ?>">
                    ...
                    <?php if ( $is_custom_list ): ?>
                        <a class="btn button show-title-form">
                            ...
                            <?php esc_html_e( 'Edit title', 'sink' ) ?>
                        </a>
                    <?php endif; ?>
                </div>
        ...
        <table class="shop_table cart wishlist_table" data-pagination="<?php echo esc_attr( $pagination ) ?>" data-per-page="<?php echo esc_attr( $per_page ) ?>" data-page="<?php echo esc_attr( $current_page ) ?>" data-id="<?php echo $wishlist_id ?>" data-token="<?php echo $wishlist_token ?>">
            ...
            <tr>
                ...
                <th class="product-name">
                    <span class="nobr"> <?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', esc_html__( 'Product Name', 'sink' ) ) ?></span>
                </th>
                
                <?php if ( $show_price ) : ?>

                    <th class="product-price">
                        <span class="nobr"> <?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Unit Price', 'sink' ) ) ?></span>
                    </th>
                ...
                <?php if ( $show_stock_status ) : ?>
                    ...
                    <th class="product-stock-stauts">
                        <span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', esc_html__( 'Stock Status', 'sink' ) ) ?></span>
                    </th>
                ...
                if ( count( $wishlist_items ) > 0 ) :
                    ...
                    foreach ( $wishlist_items as $item ) :
                        ...
                        if ( $product && $product->exists() ) :
                            ?>
                            <tr id="yith-wcwl-row-<?php echo $item[ 'prod_id' ] ?>" data-row-id="<?php echo $item[ 'prod_id' ] ?>" class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $wishlist_items, $item[ 'prod_id' ] ) ); ?>">
                                <?php if ( $show_cb ) : ?>
                                    <td data-label="" class="product-checkbox text-center">
                                ...
                                <?php if ( $is_user_owner ): ?>
                                    <td data-label="" class="product-remove text-center">
                                        <div>
                                            <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item[ 'prod_id' ] ) ) ?>" class="remove remove_from_wishlist" title="<?php esc_html_e( 'Remove this product', 'sink' ) ?>"><i class="zmdi zmdi-close zmdi-hc-fw"></i></a>
                                        </div>
                                    </td>
                                <?php endif; ?>

                                <td data-label="" class="product-thumbnail item-thumb">
                                    <a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item[ 'prod_id' ] ) ) ) ?>">
                                        <?php echo $product->get_image() ?>
                                    </a>
                                </td>

                                <td data-label="<?php echo esc_attr( apply_filters( 'yith_wcwl_wishlist_view_name_heading', esc_html__( 'Product Name', 'sink' ) ) ) ?>" class="product-name">
                                ...
                                <?php if ( $show_price ) : ?>
                                    <td data-label="<?php echo esc_attr( apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Unit Price', 'sink' ) ) ) ?>" class="product-price">
                                        <?php echo $product->get_price() ? $product->get_price_html() : apply_filters( 'yith_free_text', __( 'Free!', 'sink' ) ); ?>
                                    </td>
                                <?php endif ?>
                                
                                <?php if ( $show_stock_status ) : ?>
                                    <td data-label="<?php echo esc_attr( apply_filters( 'yith_wcwl_wishlist_view_stock_heading', esc_html__( 'Stock Status', 'sink' ) ) ) ?>" class="product-stock-status">
                                        <?php echo $stock_status == 'out-of-stock' ? '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of Stock', 'sink' ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'sink' ) . '</span>'; ?>

                                    </td>
                                <?php endif ?>
                                
                                <?php if ( $show_last_column ): ?>
                                    <td data-label="" class="product-add-to-cart text-center">
                                        <!-- Date added -->
                                        <?php
                                            if ( $show_dateadded && isset( $item[ 'dateadded' ] ) ):
                                                echo '<span class="dateadded">' . sprintf( esc_html__( 'Added on : %s', 'sink' ), date_i18n( get_option( 'date_format' ), strtotime( $item[ 'dateadded' ] ) ) ) . '</span>';
                                            endif;
                                        ?>
                                        ...
                                        <?php if ( $available_multi_wishlist && is_user_logged_in() && count( $users_wishlists ) > 1 && $move_to_another_wishlist && $is_user_owner ): ?>
                                            <select class="change-wishlist selectBox">
                                                <option value=""><?php esc_html_e( 'Move', 'sink' ) ?></option>
                                                <?php
                                                        ...
                                                        <option value="<?php echo esc_attr( $wl[ 'wishlist_token' ] ) ?>">
                                                            <?php
                                                                ...
                                                                if ( $wl[ 'wishlist_privacy' ] == 1 ) {
                                                                    $wl_privacy = esc_html__( 'Shared', 'sink' );
                                                                } elseif ( $wl[ 'wishlist_privacy' ] == 2 ) {
                                                                    $wl_privacy = esc_html__( 'Private', 'sink' );
                                                                } else {
                                                                    $wl_privacy = esc_html__( 'Public', 'sink' );
                                                                }
                                        ...
                                        <?php if ( $is_user_owner && $repeat_remove_button ): ?>
                                            <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item[ 'prod_id' ] ) ) ?>" class="remove_from_wishlist button" title="<?php esc_attr_e( 'Remove this product', 'sink' ) ?>"><?php esc_html_e( 'Remove', 'sink' ) ?></a>
                                        <?php endif; ?>
                                    </td>
                                <?php endif; ?>
                            </tr>
                    ...
                    <tr>
                        <td colspan="<?php echo esc_attr( $column_count ) ?>" class="wishlist-empty"><?php esc_html_e( 'No products were added to the wishlist', 'sink' ) ?></td>
                    </tr>
                    <?php
                endif;
                ...
                <td colspan="<?php echo esc_attr( $column_count ) ?>">
                    <?php if ( $show_cb ) : ?>
                        <div class="custom-add-to-cart-button-container">
                            <a href="<?php echo esc_url( add_query_arg( array( 'wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_meta[ 'wishlist_token' ] ) ) ) ?>" class="button alt" id="custom_add_to_cart"><?php echo apply_filters( 'yith_wcwl_custom_add_to_cart_text', esc_html__( 'Add the selected products to the cart', 'sink' ) ) ?></a>
                        </div>
                    <?php endif; ?>
                    ...
                    <?php if ( $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
                        <div class="ask-an-estimate-button-container">
                            <a href="<?php echo ( $additional_info || ! is_user_logged_in() ) ? '#ask_an_estimate_popup' : $ask_estimate_url ?>" class="btn button ask-an-estimate-button" <?php echo ( $additional_info ) ? 'data-rel="prettyPhoto[ask_an_estimate]"' : '' ?> >
                                ...
                                <?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', esc_html__( 'Ask for an estimate', 'sink' ) ) ?>
                            </a>
                        </div>
                    <?php endif; ?>
...
<?php if ( $show_ask_estimate_button && ( ! is_user_logged_in() || $additional_info ) ): ?>
    <div id="ask_an_estimate_popup">
        <?php if ( ! is_user_logged_in() ): ?>
            <label for="reply_email"><?php echo apply_filters( 'yith_wcwl_ask_estimate_reply_mail_label', esc_html__( 'Your email', 'sink' ) ) ?></label>

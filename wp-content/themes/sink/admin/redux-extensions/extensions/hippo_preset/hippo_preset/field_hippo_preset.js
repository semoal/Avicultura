...
(function ($) {
    ...
    redux.field_objects.hippo_preset = redux.field_objects.hippo_preset || {};

    $(document).ready(
        function () {
            //redux.field_objects.hippo_preset.init();

            $('#info-preset_change_warning').slideUp('slow');
        }
    );

    redux.field_objects.hippo_preset.init = function (selector) {

        if (!selector) {
            selector = $(document).find('.redux-container-hippo_preset');
        }

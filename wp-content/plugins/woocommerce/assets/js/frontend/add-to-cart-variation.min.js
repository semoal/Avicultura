...
;(function ( $, window, document, undefined ) {
	...
	VariationForm.prototype.onFoundVariation = function( event, variation ) {
		var form           = event.data.variationForm,
			...
			template       = false,
			$template_html = '',
			idProducto    = form.$form.attr('data-product_id'),
			buttonBuy =  form.$form.find('.single_add_to_cart_button')[0],
			buttonRead = form.$form.find('.single_add_to_cart_button')[1];
		...
		if ( ! variation.variation_is_visible ) {
			...
			complete: function() {
				if(window.access=="true"){
			    	//Si es online y se ha comprado mostramos solo leer revista
			    	if(variation.attributes.attribute_tipo=='Online' || variation.attributes.attribute_estado=='Online'){
			    		$('button.single_add_to_cart_button').removeClass('loading');
			    		//boton comprar revista
			    		$(buttonBuy).hide();
			    		//boton leer revista
			    		$(buttonRead).show();
	...
	VariationForm.prototype.toggleResetLink = function( on ) {
		...
		if ( on ) {
			if ( this.$resetVariations.css( 'visibility' ) === 'hidden' ) {
				this.$resetVariations.css( 'visibility', 'hidden' ).hide().fadeIn();
			}
		} else {

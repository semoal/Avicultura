<?php
	...
	class ZillaLikes {
		...
		function admin_init() {
			...
			add_settings_field( 'show_on', __( 'Automatically show likes on', 'zillalikes' ), array(
				&$this,
				'setting_show_on'
			), 'zilla-likes', 'zilla-likes' );
			add_settings_field( 'exclude_from', __( 'Exclude from Post/Page ID', 'zillalikes' ), array(
				&$this,
				'setting_exclude_from'
			), 'zilla-likes', 'zilla-likes' );
			add_settings_field( 'disable_css', __( 'Disable CSS', 'zillalikes' ), array(
				&$this,
				'setting_disable_css'
			), 'zilla-likes', 'zilla-likes' );
			add_settings_field( 'ajax_likes', __( 'AJAX Like Counts', 'zillalikes' ), array(
				&$this,
				'setting_ajax_likes'
			), 'zilla-likes', 'zilla-likes' );
			add_settings_field( 'zero_postfix', __( '0 Count Postfix', 'zillalikes' ), array(
				&$this,
				'setting_zero_postfix'
			), 'zilla-likes', 'zilla-likes' );
			add_settings_field( 'one_postfix', __( '1 Count Postfix', 'zillalikes' ), array(
				&$this,
				'setting_one_postfix'
			), 'zilla-likes', 'zilla-likes' );
			add_settings_field( 'more_postfix', __( 'More than 1 Count Postfix', 'zillalikes' ), array(
				&$this,
				'setting_more_postfix'
			), 'zilla-likes', 'zilla-likes' );
			add_settings_field( 'instructions', __( 'Shortcode and Template Tag', 'zillalikes' ), array(
				&$this,
		...
		function admin_menu() {
			...
			$page_hook = add_menu_page( __( 'ZillaLikes Settings', 'zillalikes' ), 'ZillaLikes', 'update_core', 'zilla-likes', array(
				&$this,
				'settings_page'
			), $icon_url );
			add_submenu_page( 'zilla-likes', __( 'Settings', 'zillalikes' ), __( 'ZillaLikes Settings', 'zillalikes' ), 'update_core', 'zilla-likes', array(
				&$this,
				'settings_page'
			) );
			// ZillaFramework link
			add_submenu_page( 'zillaframework', 'ZillaLikes', 'ZillaLikes', 'update_core', 'zilla-likes', array(
				&$this,
				'settings_page'
			) );
		}
			...
			<div class="wrap">
				...
				<form action="options.php" method="post">
					...
					<p class="submit"><input type="submit" class="button-primary"
					                         value="<?php _e( 'Save Changes', 'zillalikes' ); ?>"/></p>
				</form>
			</div>
			<?php
		}
		...
		function enqueue_scripts() {
			...
			if ( ! $options[ 'disable_css' ] ) {
				wp_enqueue_style( 'hippo-zilla-likes', plugins_url( '/styles/zilla-likes.css', __FILE__ ) );
			}

			wp_enqueue_script( 'hippo-zilla-likes', plugins_url( '/scripts/zilla-likes.js', __FILE__ ), array( 'jquery' ) );
			wp_enqueue_script( 'jquery' );

			wp_localize_script( 'hippo-zilla-likes', 'zilla_likes', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
		}
	...
	class ZillaLikes_Widget extends WP_Widget {
		...
		function form( $instance ) {
			...
			<p>
				...
				<label
					for="<?php echo $this->get_field_id( 'display_count' ); ?>"><?php _e( 'Display like counts' ); ?></label>
			</p>
